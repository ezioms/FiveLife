...
mp.events.add({
    ...
    "sKeys-E" : (player) => {
        ...
        if (player.job.name && player.job.name !== player.canOpen.job) return player.notify(`~r~Sie arbeiten bereits in einem anderen Job!`);
        const job = getJobByName(player.canOpen.job);
...
class Job {
    ...
    startWork(player) {
        player.outputChatBox(`!{0, 200, 0}Sie haben ihren Job bei ${this.name} angenommen`);
    ...
    finishWork(player) {
        player.job = {};
        player.outputChatBox(`!{200, 0, 0}Sie haben ihren Job ${this.name} beendet`);
