...
class faction {
	...
	createEvents() {
		...
		mp.events.add({
			"playerEnterColshape" : (player, shape) => {
				...
				player.notify(`Drücken Sie ~b~ E ~s~ um die Kleidung zu wechseln`);
			},
	...
	isDistanceRight(player1, player2, showMessToPlayer2 = false) {
		...
		player1.notify(`~r~${player2.name} ist zu weit weg von dir!`);
		if (showMessToPlayer2) player2.notify(`~r~${player1.name} ist zu weit weg von dir!`);
		return false;
	}
	...
	setRank(leader, id, value) {
		...
		leader.outputChatBox(`!{0, 225, 0}Du hast ${player.name} einen neuen Rang zugewiesen : ${value}`);
		player.outputChatBox(`!{0, 225, 0}${leader.name} hat dir den Rang ${value} zugewiesen`);
	...
	invite(leader, id) {
		...
		this.savePlayerData(player);
		player.outputChatBox(`!{0, 225, 0}${leader.name} lud dich ein zu ${this.name}`);
		leader.notify(`~g~Erfolgreich!`);
	...
	setAsLeader(admin, id) {
		...
		this.savePlayerData(player);
		player.outputChatBox(`!{0, 225, 0}Jetzt bist du Anführer von der Fraktion ${this.name}`);
		this.updateClothingMarker(player);
		admin.notify(`~g~Erfolgreich!`);
	...
	uninvite(leader, id) {
		...
		player.outputChatBox(`!{225, 0, 0}${leader.name} hat dich aus der Fraktion ${this.name} geworfen`);
		leader.notify(`~g~Erfolgreich!`);
		this.updateClothingMarker(player);
	...
	setCurrentClient(player, client) {
		...
		if (lastOfferTime && time < 60) {
			player.notify(`Warte ${60 - time} Sekunden`);
			return false;
		}
