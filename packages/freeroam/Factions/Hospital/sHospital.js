...
class Hospital extends sFaction {
    ...
    spawnAfterDeath(player) {
        ...
        player.newFine(pay, `Krankenhaus Aufenthalt`);
    ...
    openInteractionMenu(player) {
        ...
        player.call("cHospital-ShowDoctorMenu", [execute]);
    }

    rejectDoctorOffer(patient, id) {
        const doctor = mp.players.at(id);
        if (!doctor || !this.isSellerClientRight(doctor, patient)) return;
        patient.notify(`Sie haben das Angebot vom Arzt ${doctor.name} abgelehnt!`);
        doctor.notify(`${patient.name} hat Ihr Angebot abgelehnt!`);
    ...
    tryIncreaseHealingSpeed(doctor, id) {
        ...
        if (!patient.healingSpeed) {
            doctor.notify(`~r~${patient.name} geht es nicht gut!`);
            patient.notify(`~r~Ihnen geht es nicht gut!`);
            return;
        }
        if (!this.setCurrentClient(doctor, patient)) return;
        let execute = `app.whoName = '${doctor.name}';`;
        execute += `app.whoId = ${doctor.id};`;
        execute += `app.wantText = 'Möchte den Heilungsprozess beschleunigen';`;
    ...
    async successConfirmEvent(doctor, patient, price, healSpeed) {
        ...
        this.resetCurrentSeller(patient);
        this.resetCurrentClient(doctor);
        doctor.notify(`~g~${patient.name} Sie haben das Angebot bestätigt!`);
        patient.notify(`~g~Sie haben das Angebot vom Arzt ${doctor.name} bestätigt!`);
    ...
    tryHeal(doctor, id) {
        ...
        if (!this.isInThisFaction(doctor) || !this.isWorking(doctor) || !patient || !this.isDistanceRight(doctor, patient)) return;
        if (!patient.healingSpeed) {
            doctor.notify(`~r~${patient.name} ist in keiner guten Verfassung!`);
            patient.notify(`~r~Sie sind in keiner guten Verfassung!`);
            return;
        }
        if (!this.setCurrentClient(doctor, patient)) return;
        let execute = `app.whoName = '${doctor.name}';`;
        execute += `app.whoId = ${doctor.id};`;
        execute += `app.wantText = 'möchte sie behandeln';`;
...
function loadUser(player) {
    ...
    player.stopHealing = function() {
        ...
        this.healingSpeed = 0;
        this.outputChatBox(`!{0, 200, 0} Die Behandlung war erfolgreich!`);
    ...
    player.startHealing = function() {
        ...
        player.outputChatBox(`!{0, 200, 0} Mit Ihrer Behandlung wurde begonnen!`);
