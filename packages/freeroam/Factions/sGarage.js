...
class sGarage extends sBuilding {
    ...
    enteredBuildingShape(player, entranceId) {
        if (entranceId === this.mainEnter.out.entranceId) {
            ...
            player.notify(`Drücken Sie ~b~ E ~s~ um die Garage zu betreten`);
        }
        else if (this.isExitShapeValid(this.garage.shapesList, entranceId)) {
            if (!player.isDriver()) return;
            player.notify(`Drücken Sie ~b~ E ~s~ um die Garage zu verlassen`);
        }
        else if (this.isExitShapeValid(this.lift.shapesList, entranceId)) {
            player.notify(`Drücken Sie ~b~ E ~s~ um den Aufzug zu rufen`);
        }
    }
    ...
    isCheckShapeClear(player, shape) {
        ...
        for (const vehicle of vehicles) {
            if (shape.isPointWithin(vehicle.position) && shape.dimension === vehicle.dimension) {
                player.notify(`~r~Ein Fahrzeug blockiert den Bereich!`);
                return false;
            }
        }
        ...
        for (const p of players) {
            if (shape.isPointWithin(p.position)  && shape.dimension === p.dimension) {
                player.notify(`~r~Eine Person blockiert das Gebiet!`);
                return false;
            }
        }
        return true;
    ...
    showGarageMenu(player) {
        ...
        player.call("cGarage-ShowVisitorsGarageMenu", [execute, JSON.stringify(this.garage.basic.camData)]);
    }
    ...
    showLiftMenu(player) {
        ...
        player.call("cGarage-ShowVisitorsLiftMenu", [execute]);
    }
}
