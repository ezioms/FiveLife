...
class Prison {
	constructor() {
		mp.events.add({
			"playerDeath" : (player, reason, killer) => {
				...
				this.addViolation(killer, 5, "Sie haben einen Mord begangen");
			},
	...
	async loadUser(player) {
		...
		player.startJail = function() {
			if (this.jail.violations.length === 0) return this.notify("Sie sind straffrei!");
			player.tp(building.secondEntranceData.inPos);
			let jailTime = 0;
			for (const violation of this.jail.violations) {
				this.outputChatBox(`${violation.text} | ${violation.time} Monate`);
				jailTime += violation.time;
			}
			this.outputChatBox(`Gesammte Haftzeit: !{225, 0, 0}${jailTime} Monate`);
			this.jail.inside = 1;
		...
		player.jailEvent = function() {
			...
			this.notify(`Verbleibende Haftzeit: ~r~${this.jail.time} Monate`);
		...
		player.endJail = function() {
			...
			this.heading = 263.45;
			this.jail.inside = 0;
			this.outputChatBox(`!{0, 225, 0}Sie sind ein freier Mensch. Halten Sie sich an das Gesetz!`);
			this.updateWantedLevel();
	...
	escapeEvent(player) {
		player.addViolation(player.jail.time * 3, 'Ausbruch');
